<?php
    ...
    try {
        ...
        try {
            // REVERTIR STOCK (Devolver lo que se llevó antes)
            ...
            // VALIDACIÓN DE STOCK MÍNIMO (MODIFICACIÓN)
            ...
            foreach ($productos_carrito as $prod) {
                ...
                if ($producto_db) {
                    $stock_real_actual = $producto_db['cantidad']; 
                    $cantidad_nueva = $prod['cantidad'];
                    $stock_restante = $stock_real_actual - $cantidad_nueva;

                    if ($stock_real_actual <= 10) {
                        throw new Exception("Stock insuficiente: '{$producto_db['nombre']}' tiene 10 o menos unidades.");
                    }
                    if ($stock_restante < 10) {
                        throw new Exception("Stock insuficiente: Vender '{$producto_db['nombre']}' dejaría menos de 10 unidades.");
                    }
                }
            }
            ...
            //INSERTAR NUEVOS Y RESTAR STOCK
